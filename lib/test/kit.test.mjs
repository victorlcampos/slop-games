...
import { measure } from '../src/viewport.js';
...
scenario('i18n: the language is the choice, then the browser, then the fallback', () => {
  ...
  checkEqual(pickLang(null, ['fr-FR', 'de'], 'pt'), 'pt', 'a language we do not ship falls back');
  checkEqual(pickLang('klingon', [], 'pt'), 'pt', 'a stored value we do not ship is ignored');
  checkEqual(pickLang(null, [], 'pt'), 'pt', 'no signal at all: fallback');
});
...
scenario('i18n: every shipped language is one we can draw a flag for', () => {
  checkEqual(LANGS, ['pt', 'en'], 'two languages, two flags');
});
