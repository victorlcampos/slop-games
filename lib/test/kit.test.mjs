...
scenario('cutscene: the film plays to the end exactly once, and time turns the pages', () => {
  const reel = [
    { duration: 3, line: { pt: 'um', en: 'one' }, draw() {} },
    { duration: 4, draw() {} },
    { duration: 3, draw() {} },
  ];
  let ended = 0;
  const cut = createCutscene(reel, { onDone: () => ended++ });
  checkEqual(cut.scene, 0, 'a film starts on its first scene');

  cut.update(3.5);
  checkEqual(cut.scene, 1, 'the first page did not turn on its own');

  for (let t = 0; t < 12 && !ended; t += 1 / 30) cut.update(1 / 30);
  check(ended === 1 && cut.done, 'left alone, the film never ended');
  cut.update(1);
  checkEqual(ended, 1, 'the film ended twice');
});

scenario('cutscene: a click turns the page, the last click ends it, ESC always ends it', () => {
  const reel = [{ duration: 9, draw() {} }, { duration: 9, draw() {} }];
  let ended = 0;
  let turned = 0;
  const cut = createCutscene(reel, { onDone: () => ended++, onAdvance: () => turned++ });
  cut.click();
  checkEqual(cut.scene, 1, 'a click should turn the page');
  checkEqual(turned, 1, 'the page-turn hook is where the game clicks a sound');
  cut.click();
  checkEqual(ended, 1, 'clicking past the last page should end the film');

  let skipped = 0;
  const cut2 = createCutscene(reel, { onDone: () => skipped++ });
  cut2.update(1 / 30);
  cut2.skip();
  check(skipped === 1 && cut2.done, 'ESC is a promise');
});

scenario('cutscene: the skin is the default unless the game brings its own', () => {
  // a context that accepts every call and counts a few — enough to prove the
  // machine paints scene, caption, hint and markers in one frame
  const calls = { fillText: 0, arc: 0 };
  const ctx = new Proxy({}, {
    get: (o, k) => {
  ...
  const reel = [{ duration: 5, line: { pt: 'oi', en: 'hi' }, draw() {} }];
  const cut = createCutscene(reel, { onDone() {} });
  cut.update(1);
  cut.draw(ctx, 1280);
  check(calls.fillText > 0, 'the default skin never wrote the caption');
  check(calls.arc > 0, 'the default skin never drew the scene markers');

  // a game with its own engine replaces the skin wholesale, and the machine
  // hands every piece to it instead of painting
  let skinned = 0;
  let framed = 0;
  const cut2 = createCutscene(reel, {
    onDone() {},
    caption: () => skinned++,
    hint: () => skinned++,
    marker: () => skinned++,
    frame: (c, paint) => { framed++; paint(); },
  });
  cut2.update(1);
  cut2.draw(ctx, 1280);
  checkEqual(framed, 1, "the game's frame wrap has to be applied");
  checkEqual(skinned, 3, 'caption, hint and one marker should all come from the game');
});

scenario('cutscene: the caption speaks the language of the i18n instance it was given', () => {
  const i18n = createI18n({ preferred: ['pt'], dict: {} });
  let drawn = null;
  const reel = [{ duration: 5, line: { pt: 'olá', en: 'hello' }, draw() {} }];
  const cut = createCutscene(reel, { onDone() {}, i18n, caption: (c, w, text) => { drawn = text; }, hint() {}, marker() {} });
  cut.update(1);
  cut.draw({}, 1280);
  checkEqual(drawn, 'olá', 'the caption ignored the language choice');
});
