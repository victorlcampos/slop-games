...
export async function build(cfg) {
  ...
  if (pwaData) {
    const built = injectPWA(tpl, pwaData, out);
    tpl = built.html;
  ...
  if (manifestFile) {
    fs.writeFileSync(fromRoot(path.join(path.dirname(out), manifestFile.name)), manifestFile.json);
  }
 ...
 * flag. So the manifest speaks English — the product default — and the in-game
 * `<title>`, which JavaScript *can* update, is what follows the player's choice.
 */
function readGameJson(fromRoot) {
  try {
    const g = JSON.parse(fs.readFileSync(fromRoot('game.json'), 'utf8'));
    const pick = (v) => (v && typeof v === 'object' ? v.en || v.pt : v);
    return {
      name: pick(g.name),
      short: pick(g.name),
      emoji: g.emoji,
...
/**
 * Makes the page installable on a phone's home screen. The icon is an SVG
 * carrying the emoji, embedded — nothing here is a binary in a folder.
 *
 * The manifest has two forms, and the difference is not cosmetic:
 *
 *   `file: 'app.webmanifest'`  writes it next to the page and links it
 *   (default)                  embeds it as a `data:` URI, single file
 *
 * **A `data:` manifest cannot carry a relative `start_url` or `scope`**, and
 * this is not a theory — asked through the devtools protocol, Chromium answers
 * `property 'start_url' ignored, URL is invalid` and the same for `scope`. The
 * relative URLs inside a manifest resolve against the *manifest's* URL, and
 * `data:` is not a base you can resolve against. The browser then falls back to
 * the page it was installed from: install from the index and the scope lands on
 * the catalog by luck, install from inside a game and the app is scoped to that
 * game, with the catalog opening outside it.
 *
 * So anything that wants a real scope — the catalog, which owns the games under
 * it — asks for the file. A lone game embeds it and keeps its single file: it
 * has nothing under it to scope anyway.
 */
function injectPWA(tpl, pwa, out = 'dist/index.html') {
  const {
    name,
    short = name,
    emoji = '🎮',
  ...
  // a file keeps `start_url` and `scope` resolvable; the data: URI keeps the
  // page a single file, and silently loses both (see above)
  const href = file ? './' + file : 'data:application/manifest+json,' + encodeURIComponent(JSON.stringify(manifest));
  ...
  return {
    html: tpl.replace('</head>', tags + '\n</head>'),
    manifest: file ? { name: file, json: JSON.stringify(manifest, null, 2) } : null,
  };
}
