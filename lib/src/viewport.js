...
export function createViewport(canvas, options = {}) {
   ...
   */
  vp.resize = () => {
    const winWidth = Math.max(1, window.innerWidth);
    const winHeight = Math.max(1, window.innerHeight);

    // A board that only works lying down used to put up a "turn your device"
    // card. That asks the player to unlock rotation on their phone before they
    // can play — a wall in front of the game, for something the game can do
    // itself. Upright, the canvas is laid on its side instead: the game gets the
    // landscape it needs and the player keeps their phone as they are holding it.
    vp.turned = !!cfg.landscape && winHeight > winWidth;
    const cssWidth = vp.turned ? winHeight : winWidth;
    const cssHeight = vp.turned ? winWidth : winHeight;

    // the ceiling follows the LONG side, which is the game's width whichever way
    // the phone is held — a tablet lying down is not a phone standing up
    ...
    if (vp.turned) {
      // a quarter turn clockwise around the top-left corner, slid back into the
      // window: the canvas is winHeight x winWidth and lands exactly on it
   ...
   *
   * With the canvas laid on its side the finger has to be turned with it, and
   * `getBoundingClientRect` is no help: on a rotated element it gives the
   * bounding box, which is the whole window. The window's own measurements are
   * the honest ones there.
   */
  vp.point = (clientX, clientY) => {
    if (vp.turned) return turnedPoint(clientX, clientY, window.innerWidth, vp.scale);
 ...
 *
 * `landscape` is the game saying its board only works lying down: upright, the
 * two measurements are swapped here and the canvas is turned by `resize`.
 */
export function measure(cssWidth, cssHeight, rawDPR, touch, options = {}) {
  const cfg = { ...DEFAULTS, ...options };
  const turned = !!cfg.landscape && cssHeight > cssWidth;
  const w = turned ? cssHeight : cssWidth;
  const h = turned ? cssWidth : cssHeight;
  const ceiling = touch && w <= cfg.phoneWidth ? cfg.maxDPRPhone : cfg.maxDPR;
  return {
    W: Math.round(Math.min(cfg.maxWidth, Math.max(cfg.minWidth, (w / h) * cfg.height))),
    H: cfg.height,
...
/**
 * A finger on a canvas that was laid on its side. The screen's y becomes the
 * game's x, and the screen's x — measured from the right edge — becomes its y.
 * Pure, because getting it wrong is the kind of bug that makes a game look deaf
 * to the touch rather than crash.
 */
