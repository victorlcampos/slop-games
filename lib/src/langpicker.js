 ...
 *
 * The touch targets are padded past the drawing, so with a small gap they
 * overlap — and returning the first match handed the whole seam to whichever
 * flag is drawn first. The nearest centre wins instead, which splits the seam
 * down the middle whatever the caller's width and gap happen to be.
 */
export function pickLangAt(zones, x, y) {
  ...
  for (const z of zones || []) {
    ...
    const d = dx * dx + dy * dy;
    if (d < bestD) { bestD = d; best = z.lang; }
  }
  return best;
}
