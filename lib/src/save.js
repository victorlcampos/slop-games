...
export function createSave(cfg) {
  ...
  const api = {
    ...
    /**
     * Write a snapshot. Returns **whether it persisted**, not the snapshot —
     * `best = vault.save({...})` looks right, reads right, and leaves you
     * holding `true`, which is how two games in this repository ended up
     * showing "best: undefined" and never keeping a high score again.
     *
     * @returns {boolean}
     */
