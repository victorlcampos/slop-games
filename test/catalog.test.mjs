...
scenario('the catalog is installable: a manifest, an icon and a scope over the games', () => {
  const html = readFileSync(path.join(DIST, 'index.html'), 'utf8');
  const link = html.match(/<link\b[^>]*rel="manifest"[^>]*href="data:application\/(?:manifest\+)?json[^"]*,([^"]+)"/i);
  check(link, 'no manifest embedded as a data: URI — the catalog is not installable');
  const manifest = JSON.parse(decodeURIComponent(link[1]));
