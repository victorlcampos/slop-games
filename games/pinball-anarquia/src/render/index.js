...
//   parts     → everything that moves, projected
//   glass     → the sheen and the vignette that put it all under glass
//   panel     → the backglass, upright beside the table
//
// The two caches are what make this affordable: the felt texture is painted
// once, and its warp is computed once per resize. A frame after that is a
// single blit plus the live lamps.

import { C, FRAME, H, PANEL } from '../config.js';
...
export function createRenderer(t) {
  ...
  /**
   * Paint the flat playfield, then pull it into perspective one destination
   * row at a time.
   *
   ...
   * unpainted gaps wherever the far end compresses, and they show up as a
   * comb of dark lines across the top of the table.
   */
  function build(k) {
    const flat = makeCanvas(Math.ceil(v.srcW * k), Math.ceil(v.srcH * k));
    const fg = flat.getContext('2d');
    fg.scale(k, k);
    paintFelt(fg);

    const ox = v.cx - v.halfW - 6;
    const oy = v.top - 3;
    const w = Math.ceil((v.halfW * 2 + 12) * k);
    const h = Math.ceil((v.bottom - v.top + 8) * k);
    ...
    for (let py = 0; py < h; py++) {
      ...
      // sample as many source rows as this destination row is covering, so the
      ...
      const destW = row.w * k;
      const destX = (v.cx - row.w / 2 - ox) * k;
      g.drawImage(flat, 0, tRow * v.srcH * k, flat.width, srcH, destX, py, destW, 1.02);
    }
    warpOrigin = { x: ox, y: oy };
  }

  function draw(ctx, game, vp, { now = 0, attract = false } = {}) {
    const fit = Math.min(1, vp.W / FRAME);
    const ox = (vp.W - FRAME * fit) / 2;
    ...
    const k = vp.scale * vp.dpr * fit;
    const key = k.toFixed(2);
    if (!warped || builtAt !== key) {
      builtAt = key;
      build(k);
    }
    ...
    paintPanel(ctx, game, now, attract, t);

    ctx.restore();
  }

  function paintSparks(ctx, P) {
    for (let i = sparks.length - 1; i >= 0; i--) {
      ...
      s.y += s.vy / 60;
      s.vy += 14;
      const p = P.at(s.x, s.y);
      const a = s.life / s.max;
      glow(ctx, s.color, p.x, p.y, 9 * p.s, a * 0.8);
  ...
  return { draw, spark, projection: P };
}
...
function room(ctx, vp, now) {
  ...
  const spill = ctx.createRadialGradient(vp.W * 0.24, 300, 40, vp.W * 0.24, 300, 520);
...
/** The wooden box the playfield is sunk into, plus the backboard behind it. */
function cabinet(ctx, c, now) {
  const near = 26;
  const far = near * 0.62;
  ...
  // the title stencilled on it, catching the table's light
  ctx.save();
  ctx.textAlign = 'center';
  ctx.font = '900 22px "Segoe UI", system-ui, sans-serif';
  ctx.fillStyle = alpha(C.purple, 0.5 + 0.12 * Math.sin(now * 2));
  ctx.fillText('Ⓐ  N  A  R  C  H  Y', (c.tl.x + c.tr.x) / 2, c.tl.y - bbH * 0.36);
  ctx.restore();

  // the rim, a trapezoid of brushed metal around the playfield
  ctx.beginPath();
...
/** The inner faces of the side walls — the part that makes it a box and not
 *  a picture of a box. Brighter where they catch the playfield's light. */
function walls(ctx, c) {
  const near = 26;
  const far = near * 0.62;

  for (const [inner, outer, dir] of [
    [[c.bl, c.tl], [near, far], -1],
    [[c.br, c.tr], [near, far], 1],
  ]) {
    const [b, t2] = inner;
    ctx.beginPath();
    ctx.moveTo(b.x, b.y);
    ctx.lineTo(t2.x, t2.y);
    ctx.lineTo(t2.x + outer[1] * dir, t2.y - 8);
    ctx.lineTo(b.x + outer[0] * dir, b.y + 14);
    ctx.closePath();
    const face = ctx.createLinearGradient(b.x, b.y, b.x + outer[0] * dir, b.y);
    ...
    // the lit lip where wall meets felt
    ctx.beginPath();
    ctx.moveTo(b.x, b.y);
    ctx.lineTo(t2.x, t2.y);
...
function lockdownBar(ctx, c) {
  const y = c.bl.y + 4;
  ctx.beginPath();
  ctx.moveTo(c.bl.x - 30, y + 20);
  ctx.lineTo(c.br.x + 30, y + 20);
  ctx.lineTo(c.br.x + 24, y);
  ctx.lineTo(c.bl.x - 24, y);
  ctx.closePath();
  const bar = ctx.createLinearGradient(0, y, 0, y + 20);
...
function glass(ctx, c) {
  ...
  ctx.fillStyle = sheen;
  ctx.fillRect(c.tl.x - 40, c.tl.y - 10, c.br.x - c.tl.x + 80, c.bl.y - c.tl.y + 20);

  const midX = (c.bl.x + c.br.x) / 2;
  const midY = (c.tl.y + c.bl.y) / 2;
  const vig = ctx.createRadialGradient(midX, midY, 200, midX, midY, 520);
...
function tiltSlam(ctx, c, now) {
  ...
  ctx.save();
  ctx.textAlign = 'center';
  ctx.font = '900 108px "Segoe UI", system-ui, sans-serif';
  glow(ctx, C.red, cx, cy - 30, 300, 0.8);
  ctx.lineWidth = 6;
