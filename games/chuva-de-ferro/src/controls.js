...
//
// Pushing the stick up jumps and pushing it down crouches, so walking crouched
// under a cave is one thumb held diagonally.
...
export function createTouchControls(width = () => 1280) {
  ...
  const trigger = { on: false, id: null, x: 0, y: 0 };
  ...
  function start(id, x, y) {
    if (x < width() / 2) {
      ...
      trigger.on = true; trigger.id = id; trigger.x = x; trigger.y = y;
    }
  }

  function move(id, x, y) {
    if (stick.on && stick.id === id) { stick.x = x; stick.y = y; }
    if (trigger.on && trigger.id === id) { trigger.x = x; trigger.y = y; }
  }

  function end(id) {
    if (stick.on && stick.id === id) { stick.on = false; stick.id = null; }
    if (trigger.on && trigger.id === id) { trigger.on = false; trigger.id = null; }
  }
  ...
  function read() {
    ...
    return { ...asked, up: false, fire: trigger.on };
  }
