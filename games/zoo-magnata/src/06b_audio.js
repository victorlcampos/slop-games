...
const GESTO_PLANO = {
  ...
  giraffe: 'bufo', equine: 'relincho', bovine: 'mugido', ungulate: 'balido',
  camelid: 'balido', primate: 'grito', bird: 'piado', wader: 'piado',
  penguin: 'grasnado', snake: 'chiado', lizard: 'reptil', turtle: 'reptil',
  amphibian: 'coaxo', fish: 'assobio', seal: 'bufoFoca', bat: 'guincho',
  insect: 'zumbido', rodent: 'guincho', mustelid: 'guincho', swine: 'grunhido',
  kangaroo: 'grunhido', sloth: 'guincho', rhino: 'grunhido', hippo: 'grunhido',
};
const GESTO_ESPECIE = {
...
const gestoDe = sp => GESTO_ESPECIE[sp.key] || GESTO_PLANO[sp.plan] ||
...
const SFX = {
  ...
  vozHumana(p, opts) {
    ...
    if (p.role === 'trat') {            // tratador assobia, como quem chama bicho
      ...
      this._tom({ f: 1850, f2: 1400, kind: 'sine', dur: .2, vol: v * .45, t: .15, dest });
      return;
    }
  ...
  play(name) {
    switch (name) {
      ...
      case 'abrir':     if (!this._passa(name, 80)) return; this._acorde([440, 660], { dur: .16, vol: .1, passo: .04 }); break;
      case 'fechar':    if (!this._passa(name, 80)) return; this._acorde([660, 440], { dur: .14, vol: .09, passo: .04 }); break;
      case 'erro':      if (!this._passa(name, 180)) return; this._tom({ f: 190, f2: 110, kind: 'square', dur: .17, vol: .16 }); break;
      case 'trilha':    if (!this._passa(name, 55)) return; this._ruido({ dur: .07, vol: .16, f: 1600, f2: 500 }); break;
      case 'terreno':   if (!this._passa(name, 55)) return; this._ruido({ dur: .1, vol: .1, kind: 'bandpass', f: 700, f2: 1500, Q: .7 }); break;
      case 'predio':    if (!this._passa(name, 90)) return;
        this._ruido({ dur: .16, vol: .22, f: 700, f2: 160 }); this._tom({ f: 150, f2: 80, kind: 'triangle', dur: .2, vol: .16 }); break;
      case 'construir': if (!this._passa(name, 120)) return;
        this._ruido({ dur: .22, vol: .2, f: 1200, f2: 260 });
        this._acorde([330, 440, 550], { kind: 'triangle', dur: .28, vol: .12, passo: .06 }); break;
      case 'ampliar':   if (!this._passa(name, 120)) return;
        this._ruido({ dur: .16, vol: .16, f: 900, f2: 300 });
        this._acorde([392, 523], { kind: 'triangle', dur: .24, vol: .12, passo: .07 }); break;
      case 'demolir':   if (!this._passa(name, 90)) return;
        this._ruido({ dur: .3, vol: .26, f: 1800, f2: 120, Q: .6 }); this._tom({ f: 110, f2: 55, kind: 'square', dur: .18, vol: .1 }); break;
      case 'moeda':     if (!this._passa(name, 70)) return;
        this._tom({ f: 1180, dur: .07, vol: .1 }); this._tom({ f: 1560, dur: .1, vol: .09, t: .05 }); break;
      case 'compra':    if (!this._passa(name, 150)) return;
        this._tom({ f: 880, dur: .08, vol: .13 }); this._tom({ f: 1320, dur: .1, vol: .12, t: .07 });
        this._tom({ f: 1760, dur: .16, vol: .1, t: .14 }); break;
      case 'nascimento': if (!this._passa(name, 400)) return;
        this._acorde([523, 659, 784, 1046], { kind: 'triangle', dur: .3, vol: .13, passo: .09 }); break;
      case 'morte':     if (!this._passa(name, 400)) return;
        this._acorde([392, 330, 262], { kind: 'sine', dur: .5, vol: .13, passo: .16 }); break;
      case 'doente':    if (!this._passa(name, 400)) return;
        this._tom({ f: 420, f2: 300, kind: 'triangle', dur: .35, vol: .12, vib: 22, vibF: 9 }); break;
      case 'alarme':    if (!this._passa(name, 700)) return;
        for (let i = 0; i < 3; i++) {
          this._tom({ f: 780, kind: 'square', dur: .13, vol: .12, t: i * .22 });
          this._tom({ f: 560, kind: 'square', dur: .13, vol: .12, t: i * .22 + .11 });
        } break;
      case 'dia':       if (!this._passa(name, 900)) return;
        this._acorde([523, 784, 1046], { kind: 'sine', dur: .8, vol: .1, passo: .11 }); break;
      case 'contas':    if (!this._passa(name, 900)) return;
        this._ruido({ dur: .3, vol: .12, kind: 'highpass', f: 2600 });
        this._tom({ f: 300, f2: 180, kind: 'triangle', dur: .3, vol: .12, t: .1 }); break;
      case 'falencia':  if (!this._passa(name, 2000)) return;
        this._acorde([330, 262, 196, 131], { kind: 'sawtooth', dur: .9, vol: .12, passo: .22 }); break;
    }
  },

  /* ==========================================================================
     VOZES — um "gesto sonoro" por família, com exceções por espécie.
     O tamanho (esc) puxa a afinação para baixo e o PRNG do name dá uma
  ...
  _g_rugido(F, v, r) {       // leão, tigre, onça
    ...
    for (let i = 0; i < n; i++) {
      ...
      [[1, 1], [2, .55], [3, .3], [4, .14]].forEach(([m, g]) =>
        this._tom({ f: f0 * m * k, f2: f0 * m * k * (ult ? .6 : .82), kind: 'sawtooth',
                    t, dur, vol: v * g * (ult ? .62 : .5), atk: .06,
                    vib: f0 * m * .06, vibF: 26 + r() * 8, lp: F(1700) }));
      this._ruido({ t, dur, vol: v * .16, kind: 'bandpass',
                    f: F(600), f2: F(230), Q: .8, lp: F(1400) });
      t += dur + .1 + r() * .07;                       // o silêncio entre gemidos
    }
  },
  _g_miado(F, v) {           // felino pequeno
    this._tom({ f: F(620), f2: F(880), kind: 'sawtooth', dur: .18, vol: v * .8, atk: .03, vib: 24, vibF: 12 });
  ...
  _g_uivo(F, v) {            // lobo: sobe, sustenta, desce
    ...
    const trecho = (f1, f2, t, dur, vol, vib, vibF) =>
      [[1, 1], [2, .5], [3, .28], [4, .15]].forEach(([m, g]) =>
        this._tom({ f: f1 * m, f2: f2 * m, kind: 'sawtooth', t, dur, vol: vol * g,
    ...
    this._ruido({ dur: 1.7, vol: v * .13, kind: 'bandpass', f: b, f2: a, Q: .9, lp: F(2600) });  // sopro
  },
  _g_latido(F, v, r) {       // raposa, cão selvagem: rajadas curtas
    for (let i = 0; i < 2 + (r() * 2 | 0); i++) {
      this._tom({ f: F(520), f2: F(300), kind: 'sawtooth', dur: .1, vol: v * .9, t: i * .19, atk: .008, vib: 28, vibF: 20 });
      this._ruido({ dur: .08, vol: v * .5, kind: 'bandpass', f: 1300, f2: 500, Q: 1, t: i * .19 });
    }
  },
  ...
  _g_zurro(F, v) {           // zebra: late em duas partes
    for (let i = 0; i < 2; i++) {
      this._tom({ f: F(430), f2: F(230), kind: 'sawtooth', dur: .17, vol: v, t: i * .28, atk: .01, vib: 34, vibF: 17 });
      this._ruido({ dur: .13, vol: v * .4, kind: 'bandpass', f: 800, f2: 340, Q: 1, t: i * .28, lp: 1900 });
    }
  },
  ...
  _g_piado(F, v, r) {        // aves: varreduras curtas e agudas
    for (let i = 0; i < 3 + (r() * 3 | 0); i++) {
      ...
      this._tom({ f, f2: f * (1.4 + r() * .6), kind: 'sine', dur: .06, vol: v * .55, t: i * .095, atk: .004 });
      this._tom({ f: f * .45, f2: f * .6, kind: 'triangle', dur: .06, vol: v * .3, t: i * .095, atk: .004 });
    }
  },
  ...
  _g_chiado(F, v) {          // serpente
    this._ruido({ dur: .8, vol: v * .8, kind: 'highpass', f: 3600, f2: 6800, Q: .5 });
  },
  _g_reptil(F, v) {          // lagarto, crocodilo, tartaruga: sopro + ronco
    this._ruido({ dur: .5, vol: v * .55, kind: 'bandpass', f: F(760), f2: F(300), Q: 1.3 });
    this._tom({ f: F(105), f2: F(72), kind: 'sawtooth', dur: .45, vol: v * .6, vib: 9, vibF: 13 });
  },
  _g_coaxo(F, v, r) {        // anfíbio: pulsos secos
    const n = 4 + (r() * 3 | 0);
    for (let i = 0; i < n; i++) {   // real: ~92% da energia entre 1,5 e 3 kHz
      this._tom({ f: F(1350), kind: 'square', dur: .055, vol: v * .35, t: i * .08, atk: .004, lp: F(3600) });
      this._ruido({ t: i * .08, dur: .05, vol: v * .5, kind: 'bandpass', f: F(2200), Q: 2.2, lp: F(2900) });
    }
  },
  _g_assobio(F, v) {         // golfinho, boto
    this._tom({ f: F(1600), f2: F(2700), kind: 'sine', dur: .15, vol: v * .5, atk: .01 });
    this._tom({ f: F(2600), f2: F(1300), kind: 'sine', dur: .22, vol: v * .45, t: .14 });
    this._tom({ f: F(900), f2: F(1250), kind: 'sine', dur: .34, vol: v * .55, atk: .02 });  // corpo
    this._ruido({ dur: .34, vol: v * .12, kind: 'bandpass', f: F(1800), Q: .8, lp: F(3400) });  // sopro
    for (let i = 0; i < 5; i++)  // estalos de ecolocalização
      this._tom({ f: 5200, dur: .012, vol: v * .18, t: .36 + i * .035 });
  },
  _g_bufoFoca(F, v) {        // foca, leão-marinho
    for (let i = 0; i < 2; i++)
      this._tom({ f: F(330), f2: F(210), kind: 'sawtooth', dur: .21, vol: v * .85, t: i * .28, vib: 22, vibF: 15 });
  },
  _g_guincho(F, v, r) {      // roedor, morcego, pequenos: agudo e curto
    for (let i = 0; i < 2 + (r() * 2 | 0); i++) {
      const f = F(1500 + r() * 700);
      this._tom({ f, f2: f * .6, kind: 'triangle', dur: .08, vol: v * .5, t: i * .11, atk: .005 });
    }
  },
  _g_zumbido(F, v) {         // invertebrados
    this._ruido({ dur: .5, vol: v * .35, kind: 'bandpass', f: 4200, Q: 9 });
  },
  _g_grunhido(F, v, r) {     // porco, hipopótamo, rinoceronte e genéricos grandes
    const n = 3 + (r() * 2 | 0);
    for (let i = 0; i < n; i++) {
      this._tom({ f: F(560), f2: F(390), kind: 'square', dur: .1, vol: v * .3, t: i * .13, atk: .008 });
      this._ruido({ t: i * .13, dur: .09, vol: v * 1.0, kind: 'bandpass', f: F(2600), Q: .7 });
    }
  },
